...
if __name__ == "__main__":
    ...
    test_input = torch.randn(1, 3, 224, 224)  # MobileNet expects 224x224 images
    
    print("Starting trace of MobileNet V3 Small...")
    
    # Trace tensor IDs - expand functions_to_trace to cover more operations
    functions_to_trace = {
        'torch': ['flatten', 'cat', 'add', 'mul', 'sigmoid', 'hardsigmoid', 'hardswish', 'mean'],
        'F': ['relu', 'conv2d', 'linear', 'batch_norm', 'adaptive_avg_pool2d', 'hardswish', 'hardsigmoid'],
        'tensor': ['view', '__add__', 'add', '__add__', '__radd__', '__iadd__', '__mul__', 'squeeze', 'unsqueeze', 'mean']
    }
    
    with trace_tensor_ids(functions_to_trace=functions_to_trace, input_tensors=[test_input]) as tracer:
        with torch.no_grad():  # Disable gradient calculation for inference
            output = model(test_input)
    
    # Get the operation sequence
    operations = tracer.get_operation_sequence()
    print(f"Traced {len(operations)} operations in MobileNet V3 Small")
    
    # Print connected operations (limit to first 10 for readability)
    print("\nFirst 10 Connected Operations:")
    ...
    print(f"Number of classes: {output.size(1)}")
    print(f"Visualization saved to: {onnx_path}")
    print("Open with Netron to view the computation graph: https://netron.app/")
    
    
    # Export the actual PyTorch model to ONNX format
    print("\nExporting PyTorch MobileNet model to ONNX...")
    torch_onnx_path = "mobilenet_v3_small.onnx"
    
    # Export the model
    torch.onnx.export(
        model,                            # model being run
        test_input,                       # model input (or a tuple for multiple inputs)
        torch_onnx_path,                  # where to save the model
    ...
    print(f"PyTorch model exported to: {torch_onnx_path}")
    print("You can now compare the tracer visualization with the actual ONNX model")
    print("Open both files with Netron to view and compare: https://netron.app/")
